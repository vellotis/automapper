...
:::tip
Automapper's NestJS integration executes mapping by default asynchronously. It is possible to force it to be synchronous
by passing `{ sync: true }` option to `MapInterceptor` decorator.
:::
...
:::tip
Automapper's NestJS integration executes mapping by default asynchronously. It is possible to force it to be synchronous
by passing `{ sync: true }` option to `MapPipe` decorator.
:::
